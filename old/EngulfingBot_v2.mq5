//+------------------------------------------------------------------+
//|                                           EngulfingBot_v2.mq5    |
//|                       GBPUSD Engulfing + MA Trend Confluence     |
//|                                                       Version 2.0|
//|                    SIGNAL BROADCAST MODE - For Multi-Account Use |
//|  v2.0: Added Moving Average trend filter for confluence          |
//+------------------------------------------------------------------+
...
input int      MagicNumber = 236879;                     // Magic Number (v2 EA Account)
...
input bool     EnableMAFilter = true;                    // Enable MA trend confluence
input int      MAPeriodFast = 20;                        // Fast MA period (trend)
input int      MAPeriodSlow = 50;                        // Slow MA period (0 = disabled)
input ENUM_MA_METHOD MAMethod = MODE_EMA;                // MA calculation method
input ENUM_APPLIED_PRICE MAAppliedPrice = PRICE_CLOSE;   // Price for MA calculation
input bool     RequirePriceAboveMA = true;               // BUY: Price must be above MA
input bool     RequirePriceBelowMA = true;               // SELL: Price must be below MA
...
input string   CSVFileName = "EngulfingBot_v2_Backtest.csv"; // CSV file name
...
int g_maHandleFast = INVALID_HANDLE;
int g_maHandleSlow = INVALID_HANDLE;
...
int OnInit() {
    Log("========================================", "INIT");
    Log("Engulfing Bot v2.0 + MA Trend Filter (GBPUSD)", "INIT");
    Log("Supports: GBPUSD with MA Confluence", "INIT");
    ...
    if(EnableMAFilter) {
        g_maHandleFast = iMA(_Symbol, PERIOD_CURRENT, MAPeriodFast, 0, MAMethod, MAAppliedPrice);
        if(g_maHandleFast == INVALID_HANDLE) {
            Log("Failed to create Fast MA indicator", "ERROR");
            return INIT_FAILED;
        }

        if(MAPeriodSlow > 0) {
            g_maHandleSlow = iMA(_Symbol, PERIOD_CURRENT, MAPeriodSlow, 0, MAMethod, MAAppliedPrice);
            if(g_maHandleSlow == INVALID_HANDLE) {
                Log("Failed to create Slow MA indicator", "ERROR");
                return INIT_FAILED;
            }
        }

        Log("=== MA TREND FILTER ENABLED ===", "INIT");
        Log("Fast MA: " + IntegerToString(MAPeriodFast) + " " + EnumToString(MAMethod), "INIT");
        if(MAPeriodSlow > 0) {
            Log("Slow MA: " + IntegerToString(MAPeriodSlow) + " " + EnumToString(MAMethod), "INIT");
        }
        Log("BUY requires price above MA: " + (RequirePriceAboveMA ? "YES" : "NO"), "INIT");
        Log("SELL requires price below MA: " + (RequirePriceBelowMA ? "YES" : "NO"), "INIT");
    } else {
        Log("MA Filter: DISABLED", "INIT");
    }
...
//+------------------------------------------------------------------+
//| Check if MA trend aligns with trade direction                     |
//+------------------------------------------------------------------+
bool CheckMATrendAlignment(bool isBullish) {
    if(!EnableMAFilter) return true;  // Filter disabled = always pass
    ...
    // Get Fast MA value
    if(CopyBuffer(g_maHandleFast, 0, 1, 1, maFast) <= 0) {
        Log("Failed to get Fast MA value", "ERROR");
        return false;
    }

    double fastMA = maFast[0];
    double slowMA = 0;

    // Get Slow MA if enabled
    if(MAPeriodSlow > 0 && g_maHandleSlow != INVALID_HANDLE) {
        if(CopyBuffer(g_maHandleSlow, 0, 1, 1, maSlow) <= 0) {
            Log("Failed to get Slow MA value", "ERROR");
            return false;
        }
        slowMA = maSlow[0];
    }
    ...
    // Log MA values
    Log("MA Check: Price=" + DoubleToString(currentPrice, g_digits) +
        " | FastMA(" + IntegerToString(MAPeriodFast) + ")=" + DoubleToString(fastMA, g_digits) +
        (MAPeriodSlow > 0 ? " | SlowMA(" + IntegerToString(MAPeriodSlow) + ")=" + DoubleToString(slowMA, g_digits) : ""), "MA");

    // BULLISH trade - price should be ABOVE MA
    if(isBullish) {
        if(RequirePriceAboveMA && currentPrice < fastMA) {
            Log("REJECTED: Bullish pattern but price below Fast MA", "MA");
            return false;
        }
        // Also check price above slow MA if enabled
        if(MAPeriodSlow > 0 && currentPrice < slowMA) {
            Log("REJECTED: Bullish pattern but price below Slow MA", "MA");
            return false;
        }
        // Check fast MA > slow MA (uptrend) if dual MA enabled
        if(MAPeriodSlow > 0 && fastMA < slowMA) {
            Log("REJECTED: Fast MA below Slow MA (downtrend)", "MA");
            return false;
        }
    }
    // BEARISH trade - price should be BELOW MA
    else {
        if(RequirePriceBelowMA && currentPrice > fastMA) {
            Log("REJECTED: Bearish pattern but price above Fast MA", "MA");
            return false;
        }
        // Also check price below slow MA if enabled
        if(MAPeriodSlow > 0 && currentPrice > slowMA) {
            Log("REJECTED: Bearish pattern but price above Slow MA", "MA");
            return false;
        }
        // Check fast MA < slow MA (downtrend) if dual MA enabled
        if(MAPeriodSlow > 0 && fastMA > slowMA) {
            Log("REJECTED: Fast MA above Slow MA (uptrend)", "MA");
            return false;
        }
    }

    Log("MA CONFIRMED: Trend aligns with " + (isBullish ? "BULLISH" : "BEARISH") + " pattern", "MA");
    return true;
}
...
void OnNewBar() {
    ...
    if(DetectEngulfing(bar)) {
        // Check MA trend alignment before opening trade
        if(CheckMATrendAlignment(bar.isBullish)) {
            g_engulfing = bar;
            OpenFirstTrade();
        } else {
            Log("Pattern detected but rejected by MA filter", "FILTER");
        }
    }
}
...
void OpenFirstTrade() {
    ...
    if(BroadcastMode) {
        string pattern = g_engulfing.isBullish ? "BULLISH_ENGULFING_MA" : "BEARISH_ENGULFING_MA";
    ...
    if(ExecuteOnMaster || !BroadcastMode) {
        ...
        string comment = _Symbol + "_EngulfMA_" + (g_engulfing.isBullish ? "Buy1" : "Sell1");
        bool success;
...
void OpenSecondTrade(double lastLow, double lastHigh) {
    ...
    if(BroadcastMode) {
        string pattern = g_engulfing.isBullish ? "BULLISH_ENGULFING_MA_T2" : "BEARISH_ENGULFING_MA_T2";
        BroadcastTradeSignal(action, sl, RiskPercent, pattern);
    }
    ...
    if(ExecuteOnMaster || !BroadcastMode) {
        // Dynamic comment based on symbol
        string comment = _Symbol + "_EngulfMA_" + (g_engulfing.isBullish ? "Buy2" : "Sell2");
        bool success;
...
void OnDeinit(const int reason) {
    ...
    // Release MA indicator handles
    if(g_maHandleFast != INVALID_HANDLE) {
        IndicatorRelease(g_maHandleFast);
    }
    if(g_maHandleSlow != INVALID_HANDLE) {
        IndicatorRelease(g_maHandleSlow);
    }
